...
export default function Reports() {
  ...
  const { sessionReports, fetchReportsFromBackend } = useVM();
  ...
  // Function to refresh reports from backend
  ...
  // Debug info on mount
  useEffect(() => {
    console.log("🔍 Reports page mounting...");
    // Reports are already fetched by VMContext on mount, no need to fetch again
  }, []);
  ...
  // Debug info on mount
  useEffect(() => {
    console.log("🔍 Reports page mounted with sessionReports:", sessionReports);
  }, [sessionReports, token]);
    ...
    return sessionReports.map(report => {
      ...
      return {
        ...
        metrics: {
          ...
          ttfoMs: report.realTTFO || (Math.floor(Math.random() * 8) + 7), // Use REAL TTFO from session tracking
  ...
  }, [query, statusFilter, runs, refreshKey]); // Add refreshKey dependency
  ...
  const stats = useMemo(() => {
    ...
    if (selected) {
      // Show metrics for the selected run
      const ttfo = selected.metrics.ttfoMs;
      const totalTime = selected.metrics.totalMs;
      const successRate = selected.sessionDetails?.successRate || 0;
      const totalCommands = selected.sessionDetails?.totalCommands || 0;
      const throughput = totalCommands / Math.max(1e-9, totalTime / 60000); // commands per minute

      return { ttfo, totalTime, successRate, throughput };
    } else {
      // Show averages over filtered runs
      const list = filteredRuns;

      const avg = (arr) => arr.reduce((a, b) => a + b, 0) / Math.max(1, arr.length);

      const ttfo = avg(list.map((r) => r.metrics.ttfoMs));
      const totalTime = avg(list.map((r) => r.metrics.totalMs));
      
      // Calculate actual success rate based on command success/failure, not session status
      const totalCommands = list.reduce((sum, r) => sum + (r.sessionDetails?.totalCommands || 0), 0);
      const successfulCommands = list.reduce((sum, r) => sum + (r.sessionDetails?.successfulCommands || 0), 0);
  ...
  }, [filteredRuns, refreshKey, selected]);
        ...
        <div className="card">
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            ...
            <div>
              ...
              <p className="muted">
                ...
                Showing VM session performance metrics. Reports are automatically saved when you disconnect from a VM.
          ...
          <div className="stats-grid mt-20">
            ...
            <div className="stat-card">
              ...
              <div className="stat-label">{selected ? 'TTFO' : 'Avg TTFO'}</div>

              <div className="stat-value">{fmtMs(Math.round(stats.ttfo))}</div>
            ...
            <div className="stat-card">
              ...
              <div className="stat-label">{selected ? 'Total Time' : 'Avg Total Time'}</div>

              <div className="stat-value">{fmtMs(Math.round(stats.totalTime))}</div>
